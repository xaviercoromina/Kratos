...
namespace Kratos
{
    ...
    const Parameters ApplyRayCastingProcess<TDim>::GetDefaultParameters() const
    {
        return Parameters(R"({
            ...
            "set_interface_to_zero_distance" : false
        })");
    ...
    void ApplyRayCastingProcess<TDim>::Execute()
    {
        ...
        auto set_distance_function = GetSetNodalDistanceFunction();
    ...
    template<std::size_t TDim>
    std::function<void(const double,double&)> ApplyRayCastingProcess<TDim>::GetSetNodalDistanceFunction()
    {
        std::function<void(const double,double&)> set_distance_function;
        if (mSettings["set_interface_to_zero_distance"].GetBool()) {
