...
BinaryExpression<TOperationType>::BinaryExpression(
    ...
    : Expression(pLeft->GetNumberOfEntities()),
      mpLeft(pLeft),
      mpRight(pRight)
{
    ...
    KRATOS_ERROR_IF_NOT(pLeft->GetNumberOfEntities() == pRight->GetNumberOfEntities())
        << "Number of entities mismatch in left and right side expressions. [ left expression"
        << " number of entities = " << pLeft->GetNumberOfEntities()
        << ", right expression number of entities = "
        << pRight->GetNumberOfEntities() << " ].\n";
