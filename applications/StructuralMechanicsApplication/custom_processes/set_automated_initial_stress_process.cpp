...
void SetAutomatedInitialStressProcess::ExecuteInitialize()
{
    ...
    block_for_each(mrThisModelPart.Elements(), [&](Element &rElement) {
        ...
        hole_generatrix_axis = mThisParameters["hole_generatrix_axis"].GetVector();
        // KRATOS_WATCH(hole_generatrix_axis)  

        hole_generatrix_point = mThisParameters["hole_generatrix_point"].GetVector();

        normalized_generatrix_vector = ZeroVector();
        normalized_generatrix_vector[0] = (hole_generatrix_axis[0] * hole_generatrix_axis[0]) / sqrt(hole_generatrix_axis[0] * hole_generatrix_axis[0] + hole_generatrix_axis[1] * hole_generatrix_axis[1] + hole_generatrix_axis[2] * hole_generatrix_axis[2]);
        normalized_generatrix_vector[1] = (hole_generatrix_axis[1] * hole_generatrix_axis[1]) / sqrt(hole_generatrix_axis[0] * hole_generatrix_axis[0] + hole_generatrix_axis[1] * hole_generatrix_axis[1] + hole_generatrix_axis[2] * hole_generatrix_axis[2]);
        normalized_generatrix_vector[2] = (hole_generatrix_axis[2] * hole_generatrix_axis[2]) / sqrt(hole_generatrix_axis[0] * hole_generatrix_axis[0] + hole_generatrix_axis[1] * hole_generatrix_axis[1] + hole_generatrix_axis[2] * hole_generatrix_axis[2]);
        // KRATOS_WATCH(normalized_generatrix_vector);

        element_centroid = rElement.GetGeometry().Center();
        // KRATOS_WATCH(element_centroid)

        relative_position_vector = ZeroVector();
        ...
        intersection_point = hole_generatrix_point + vector_scaler * normalized_generatrix_vector;
        ...
        radial_position_vector = element_centroid - intersection_point;
        // KRATOS_WATCH(radial_position_vector)  

        centroid_relative_distance = sqrt(radial_position_vector[0] * radial_position_vector[0] + radial_position_vector[1] * radial_position_vector[1] + radial_position_vector[2] * radial_position_vector[2]) - hole_radius_offset;
        // KRATOS_WATCH(centroid_relative_distance)    

        // KRATOS_WATCH(mrThisModelPart.GetTable(0))
        initial_stress_vector = ZeroVector();

        // KRATOS_WATCH(mrThisModelPart.Name())
        // KRATOS_WATCH(mrThisModelPart.Tables())

        for (IndexType TableId = 0; TableId < mrThisModelPart.Tables().size(); ++TableId) {

                // KRATOS_WATCH(initial_stress_vector)
                // if  (mrThisModelPart.HasTables())  {
                initial_stress_vector[TableId] = mrThisModelPart.GetTable(TableId).GetValue(centroid_relative_distance);
                // }
