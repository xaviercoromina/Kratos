...
class GeoMechanicsAnalysis(AnalysisStage):
    ...
    def __init__(self, model, project_parameters):
        ...
        if solver_settings.Has("domain_size") and project_parameters["problem_data"].Has("domain_size"):
            raise Exception("GeoMechanicsAnalysis: " + '"domain_size" defined both in "problem_data" and "solver_settings"!')

        if solver_settings.Has("model_part_name") and project_parameters["problem_data"].Has("model_part_name"):
            raise Exception("GeoMechanicsAnalysis: " + '"model_part_name" defined both in problem_data" and "solver_settings"!')

        if solver_settings.Has("time_stepping") and project_parameters["problem_data"].Has("time_Step"):
            raise Exception("GeoMechanicsAnalysis: " + '"time_stepping" defined both in "problem_data" and "solver_settings"!')

        if not solver_settings.Has("time_stepping"):
            raise Exception("GeoMechanicsAnalysis: Using the old way to pass the time_step, this was removed!")

        if not solver_settings.Has("domain_size"):
            raise Exception("GeoMechanicsAnalysis: Using the old way to pass the domain_size, this was removed!")
    ...
    def OutputSolutionStep(self):
        ...
        if self.contact_problem:
            ...
            if is_output_step:
                ...
                KratosMultiphysics.Logger.PrintWarning("GeoMechanicsAnalysis", "STEP: ", self._GetSolver().GetComputingModelPart().ProcessInfo[KratosMultiphysics.STEP])
                KratosMultiphysics.Logger.PrintWarning("GeoMechanicsAnalysis", "TIME: ", self.time)
...
if __name__ == "__main__":
    ...
    if len(argv) > 2:
        ...
        err_msg += '    "python3 gomechanics_analysis.py CustomProjectParameters.json"\n'
