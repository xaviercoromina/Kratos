...
std::vector<Parameters> ApplyVectorConstraintsTableProcess::CreateParametersForActiveComponents(const Parameters& rSettings) const
{
...
std::vector<char> ApplyVectorConstraintsTableProcess::ActiveComponents(const Parameters& rSettings) const
{
...
Parameters ApplyVectorConstraintsTableProcess::CreateParametersForComponent(const Parameters& rSettings, char component) const
{
    Parameters result;
    const auto index = ComponentToIndex(component);
...
ApplyVectorConstraintsTableProcess::MakeProcessFor(const Parameters& rParameters) const
{
    if (rParameters.Has("table")) {
        return Kratos::make_unique<ApplyComponentTableProcess>(mrModelPart, rParameters);
    }

    return Kratos::make_unique<ApplyConstantScalarValueProcess>(mrModelPart, rParameters);
}
