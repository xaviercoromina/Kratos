#include "binbased_DEM_fluid_coupled_mapping_for_plasma_dynamics.h"
#include "../SwimmingDEMApplication/swimming_DEM_application.h"
#include "../SwimmingDEMApplication/swimming_dem_application_variables.h"
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::InterpolateFromFluidMesh(
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ImposeFlowOnDEMFromField(
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ImposeVelocityOnDEMFromFieldToAuxVelocity(
    FluidFieldUtility& r_flow,
    ModelPart& r_dem_model_part)
{
    KRATOS_TRY
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::InterpolateVelocityOnAuxVelocity(
    ModelPart& r_fluid_model_part,
    ModelPart& r_dem_model_part,
    BinBasedFastPointLocator<3>& bin_of_objects_fluid,
    const double alpha)
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::UpdateOldVelocity(ModelPart& r_dem_model_part)
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::UpdateOldAdditionalForce(ModelPart& r_dem_model_part)
{
    #pragma omp parallel for
    for (int i = 0; i < (int)r_dem_model_part.Nodes().size(); ++i){
        NodeIteratorType i_particle = r_dem_model_part.NodesBegin() + i;
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::InterpolateFromDEMMesh(
    ModelPart& r_dem_model_part,
    ModelPart& r_fluid_model_part,
    BinBasedFastPointLocator<3>& bin_of_objects_fluid) // this is a bin of objects which contains the FLUID model part
{
    KRATOS_TRY
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::VariingRadiusHomogenizeFromDEMMesh(
    ModelPart& r_dem_model_part,
    ModelPart& r_fluid_model_part,
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::HomogenizeFromDEMMesh(
    ModelPart& r_dem_model_part,
    ModelPart& r_fluid_model_part,
    const double& search_radius,
    const double& shape_factor, // it is the density function's maximum divided by its support's radius
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ComputeHomogenizedFluidFraction(ModelPart& r_fluid_model_part, ModelPart& r_dem_model_part)
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::Clear(
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CopyValues(
        ModelPart& r_model_part,
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ApplyExponentialTimeFiltering(
        ModelPart& r_model_part,
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ComputePostProcessResults(
    ModelPart& r_dem_model_part,
    ModelPart& r_fluid_model_part,
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::InterpolateFluidFraction(
    ModelPart& r_dem_model_part,
    ModelPart& r_fluid_model_part,
    BinBasedFastPointLocator<3>& bin_of_objects_fluid) // this is a bin of objects which contains the FLUID model part
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::InterpolateOtherFluidVariables(
    ModelPart& r_dem_model_part,
    ModelPart& r_fluid_model_part,
    BinBasedFastPointLocator<3>& bin_of_objects_fluid) // this is a bin of objects that contains the FLUID model part
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::SearchParticleNodalNeighbours(ModelPart& r_fluid_model_part,
                                   ModelPart& r_dem_model_part,
                                   const double& search_radius)
{
    ...
    for (int i = 0; i < (int)n_nodes; ++i){
        ElementIteratorType i_particle = r_dem_model_part.ElementsBegin() + i;
        IonParticle* p_particle = dynamic_cast<IonParticle* >(&(*i_particle));
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::SearchParticleNodalNeighboursFixedRadius(ModelPart& r_fluid_model_part,
                                   ModelPart& r_dem_model_part,
                                   const double& search_radius)
{
    unsigned int n_nodes = r_dem_model_part.Nodes().size();
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::RecalculateDistances(ModelPart& r_dem_model_part)
{
    ...
    for (int i = 0; i != n_particles; ++i){
        IonParticle* p_particle = mSwimmingSphereElementPointers[i];
...
bool BinBasedDEMFluidCoupledMappingForPlasmaDynamics::IsDEMVariable(const VariableData& var)
{
...
bool BinBasedDEMFluidCoupledMappingForPlasmaDynamics::IsFluidVariable(const VariableData& var)
{
...
bool BinBasedDEMFluidCoupledMappingForPlasmaDynamics::IsFluidVariableToBeTimeFiltered(const VariableData& var)
{
...
array_1d<double, 3> BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateAcceleration(const Geometry<Node<3> >& geom, const Vector& N)
{
...
double BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateNormOfSymmetricGradient(const Geometry<Node<3> >& geom, const int index)
{
...
array_1d<double, 3> BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateVorticity(const Geometry<Node<3> >& geom, const int index)
{
    Geometry<Node<3> >::ShapeFunctionsGradientsType DN_DX;
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateVelocityProjectedRate(
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::Project(Element::Pointer p_elem,
             const Vector& N,
             Node<3>::Pointer p_node,
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ProjectPlasmaDynamics(Element::Pointer p_elem,
             const Vector& N,
             Node<3>::Pointer p_node,
             const VariableData *r_destination_variable,
             double alpha)
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::GetElectricFieldValue(ModelPart& r_fluid_model_part)
{
    #pragma omp parallel for
    for (int i = 0; i < (int)r_fluid_model_part.Nodes().size(); ++i){
        NodeIteratorType i_node = r_fluid_model_part.NodesBegin() + i;
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::DistributeDimensionalContributionToFluidFraction(
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::Distribute(
    Element::Pointer p_elem,
    const Vector& N,
    Node<3>::Pointer p_node,
    const VariableData *r_destination_variable)
{
    if (mCouplingType == 0){
        ...
        else if (*r_destination_variable == FLUID_ION_DENSITY){
            TransferWithLinearWeighing(p_elem, N, p_node, FLUID_ION_DENSITY, MACROPARTICLE_ION_DENSITY);
        }        
    }
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ComputeHomogenizedNodalVariable(
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateFluidFraction(ModelPart& r_fluid_model_part)
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateFluidMassFraction(ModelPart& r_fluid_model_part)
{
    OpenMPUtils::CreatePartition(OpenMPUtils::GetNumThreads(), r_fluid_model_part.Nodes().size(), mNodesPartition);
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::Interpolate(
    Element::Pointer p_elem,
    const Vector& N,
    Node<3>::Pointer p_node,
    const Variable<array_1d<double, 3> >& r_origin_variable,
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::InterpolateAcceleration(
    Element::Pointer p_elem,
    const Vector& N,
    Node<3>::Pointer p_node,
    const Variable<array_1d<double, 3> >& r_destination_variable)
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::InterpolateShearRate(
    Element::Pointer p_elem,
    const Vector& N,
    Node<3>::Pointer p_node,
    const Variable<double>& r_destination_variable)
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::InterpolateVorticity(
    Element::Pointer p_elem,
    const Vector& N,
    Node<3>::Pointer p_node,
    const Variable<array_1d<double, 3> >& r_destination_variable)
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::TransferWithConstantWeighing(
    Element::Pointer p_elem,
    const Vector& N,
    Node<3>::Pointer p_node,
    const Variable<array_1d<double, 3> >& r_destination_variable,
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::TransferWithLinearWeighing(
    Element::Pointer p_elem,
    const array_1d<double,3 + 1>& N,
    Node<3>::Pointer p_node,
    const Variable<array_1d<double, 3> >& r_destination_variable,
    const Variable<array_1d<double, 3> >& r_origin_variable)
{
    ...
    const Variable<double>& r_origin_variable)
{
    ...
    if (r_origin_variable == MACROPARTICLE_ION_DENSITY){
        ...
        for (unsigned int i = 0; i < 3 + 1; ++i){
            //KRATOS_WATCH("FLUID_ION_DENSITY:");
            //KRATOS_WATCH(geom[i].FastGetSolutionStepValue(FLUID_ION_DENSITY));
            double& fluid_density = geom[i].FastGetSolutionStepValue(r_destination_variable);
            const double nodal_fluid_volume         = geom[i].FastGetSolutionStepValue(NODAL_AREA);
            const double fluid_fraction             = geom[i].FastGetSolutionStepValue(FLUID_FRACTION);
            const double particles_mass_fraction    = 1.0 - geom[i].FastGetSolutionStepValue(PHASE_FRACTION);
            const double total_particles_mass       = particles_mass_fraction / (1.0 - particles_mass_fraction) * fluid_fraction * fluid_density * nodal_fluid_volume;
            const double particle_mass              = p_node->FastGetSolutionStepValue(NODAL_MASS);
            const double weight_limit = 1.0e-15 *total_particles_mass;
            double weight;

            if (total_particles_mass >= particle_mass && particle_mass >= weight_limit) {
                weight = N[i] * particle_mass / total_particles_mass;
            }
            else {
                weight = N[i];
            ...
            if (mTimeAveragingType == 0 || mTimeAveragingType == 2){
                fluid_density += weight * origin_data;
            }

            else if (mTimeAveragingType == 1){
                const int n = std::max(1, mNumberOfDEMSamplesSoFarInTheCurrentFluidStep);
                fluid_density += (fluid_density + weight * origin_data - fluid_density) / (n + 1);
            }
            //KRATOS_WATCH("FLUID_ION_DENSITY:");
            //KRATOS_WATCH(geom[i].FastGetSolutionStepValue(FLUID_ION_DENSITY));
        }
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateNodalFluidFractionWithConstantWeighing(
    Element::Pointer p_elem,
    const Vector& N,
    ParticleType& particle)
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateNodalFluidFractionWithLinearWeighing(
    Element::Pointer p_elem,
    const Vector& N,
    ParticleType& particle)
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateNodalFluidFractionByLumpedL2Projection(
        Element::Pointer p_elem,
        const Vector& N,
        Node<3>::Pointer p_node)
{
    ...
    for (unsigned int i = 0; i < 3 + 1; ++i){
        geom[i].FastGetSolutionStepValue(FLUID_FRACTION) += (3 + 1) * N[i] * particle_volume / elemental_volume;
    }
}
//***************************************************************************************************************
//***************************************************************************************************************

void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::TransferByAveraging(
    const ParticleType& particle,
    const ResultNodesContainerType& neighbours,
    const DistanceType& weights,
    const Variable<array_1d<double, 3> >& r_destination_variable,
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateNodalFluidFractionByAveraging( // it is actually calculating its complementary here; (1 - this value) is performed later
    ParticleType& particle,
    const ResultNodesContainerType& neighbours,
    const DistanceType& weights)
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateNodalSolidFractionByAveraging(
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::MultiplyNodalVariableBy(ModelPart& r_model_part, const Variable<double>& r_variable, const double& factor)
{
    #pragma omp parallel for
    for (int i = 0; i < (int)r_model_part.Nodes().size(); ++i){
        NodeIteratorType i_node = r_model_part.NodesBegin() + i;
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::MultiplyNodalVariableBy(ModelPart& r_model_part, const Variable<array_1d<double, 3> >& r_variable, const double& factor)
{
    #pragma omp parallel for
    for (int i = 0; i < (int)r_model_part.Nodes().size(); ++i){
        NodeIteratorType i_node = r_model_part.NodesBegin() + i;
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ResetDEMVariables(ModelPart& r_dem_model_part)
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ResetFluidVariables(ModelPart& r_fluid_model_part)
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ResetFLuidVelocityRate(const NodeIteratorType& node_it)
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateFluidNodesMaxNodalArea(ModelPart& r_fluid_model_part)
{
...
inline void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::ClearVariable(const NodeIteratorType& node_it, const VariableData *var)
{
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::Clear(ModelPart& r_model_part, const Variable<double>& r_variable)
{
    #pragma omp parallel for
    for (int i = 0; i < (int)r_model_part.Nodes().size(); ++i){
        NodeIteratorType i_node = r_model_part.NodesBegin() + i;
...
inline unsigned int BinBasedDEMFluidCoupledMappingForPlasmaDynamics::GetNearestNode(const Vector& N)
{
    double max = N[0];
...
void BinBasedDEMFluidCoupledMappingForPlasmaDynamics::FillVectorOfSwimmingSpheres(ModelPart& r_dem_model_part){
    ...
    for (ElementsArrayType::iterator i_elem = r_dem_model_part.ElementsBegin(); i_elem != r_dem_model_part.ElementsEnd(); ++i_elem){
        mSwimmingSphereElementPointers[i] = &(dynamic_cast<Kratos::IonParticle&>(*i_elem));
        ++i;
    }
}
//***************************************************************************************************************
...
double inline BinBasedDEMFluidCoupledMappingForPlasmaDynamics::CalculateDistance(Node<3>::Pointer a, IonParticle* b){
...
double  BinBasedDEMFluidCoupledMappingForPlasmaDynamics::GetAlpha(const VariableData& r_variable)
{
...
const Variable<array_1d<double,3>>& BinBasedDEMFluidCoupledMappingForPlasmaDynamics::GetBodyForcePerUnitMassVariable() const
{
