...
//   Adapted version for plasma dynamics by Marc Chung To Sang, mchungtosang@cimne.upc.edu
//

#if !defined(KRATOS_BINBASED_DEM_FLUID_COUPLED_MAPPING_FOR_PLASMA_DYNAMICS)
#define  KRATOS_BINBASED_DEM_FLUID_COUPLED_MAPPING_FOR_PLASMA_DYNAMICS
...
#include "../SwimmingDEMApplication/custom_utilities/mollification/density_function_polynomial.h"
#include "../SwimmingDEMApplication/custom_utilities/custom_functions.h"
...
#include "../SwimmingDEMApplication/custom_utilities/search/point_point_search.h"

#include "utilities/binbased_fast_point_locator.h"
#include "utilities/binbased_nodes_in_element_locator.h"
#include "../SwimmingDEMApplication/custom_utilities/derivative_recovery.h"
...
class KRATOS_API(PLASMA_DYNAMICS_APPLICATION) BinBasedDEMFluidCoupledMappingForPlasmaDynamics
{
public:
///@name Type Definitions
///@{
...
typedef IonParticle                                           ParticleType;
...
/// Pointer definition of BinBasedDEMFluidCoupledMappingForPlasmaDynamics
typedef BinBasedDEMFluidCoupledMappingForPlasmaDynamics BinBasedDEMFluidCoupledMappingForPlasmaDynamics_3D_IonParticle;
KRATOS_CLASS_POINTER_DEFINITION(BinBasedDEMFluidCoupledMappingForPlasmaDynamics_3D_IonParticle);
...
BinBasedDEMFluidCoupledMappingForPlasmaDynamics(Parameters& rParameters)
...
/// Destructor.
virtual ~BinBasedDEMFluidCoupledMappingForPlasmaDynamics() {}
...
void InterpolateFromFluidMesh(ModelPart& r_fluid_model_part, ModelPart& r_dem_model_part, Parameters& parameters, BinBasedFastPointLocator<3>& bin_of_objects_fluid, const double alpha);
...
void InterpolateVelocityOnAuxVelocity(ModelPart& r_fluid_model_part, ModelPart& r_dem_model_part, BinBasedFastPointLocator<3>& bin_of_objects_fluid, const double alpha);
void UpdateOldVelocity(ModelPart& r_dem_model_part);
void UpdateOldAdditionalForce(ModelPart& r_dem_model_part);
void InterpolateFromDEMMesh(ModelPart& r_dem_model_part, ModelPart& r_fluid_model_part, BinBasedFastPointLocator<3>& bin_of_objects_fluid); // this is a bin of objects which contains the FLUID model part
...
void ComputePostProcessResults(ModelPart& r_dem_model_part, ModelPart& r_fluid_model_part, ModelPart& rfem_dem_model_part, BinBasedFastPointLocator<3>& bin_of_objects_fluid, const ProcessInfo& r_current_process_info);
...
std::vector<IonParticle* > mSwimmingSphereElementPointers;
...
void InterpolateFluidFraction(ModelPart& r_dem_model_part, ModelPart& r_fluid_model_part, BinBasedFastPointLocator<3>& bin_of_objects_fluid); // this is a bin of objects which contains the FLUID model part
void InterpolateOtherFluidVariables(ModelPart& r_dem_model_part, ModelPart& r_fluid_model_part, BinBasedFastPointLocator<3>& bin_of_objects_fluid); // this is a bin of objects which contains the FLUID model part
...
void TransferWithLinearWeighing(Element::Pointer p_elem, const array_1d<double,3 + 1>& N, Node<3>::Pointer p_node, const Variable<array_1d<double, 3> >& r_destination_variable, const Variable<array_1d<double, 3> >& r_origin_variable);
void TransferWithLinearWeighing(Element::Pointer p_elem, const array_1d<double,3 + 1>& N, Node<3>::Pointer p_node, const Variable<double>& r_destination_variable, const Variable<double>& r_origin_variable);
...
double inline CalculateDistance(Node<3>::Pointer a, IonParticle* b);
...
/// Assignment operator.
BinBasedDEMFluidCoupledMappingForPlasmaDynamics& operator=(BinBasedDEMFluidCoupledMappingForPlasmaDynamics const& rOther);

///@}

}; // Class BinBasedDEMFluidCoupledMappingForPlasmaDynamics

/// output stream function

inline std::ostream& operator << (std::ostream& rOStream,
                                  const BinBasedDEMFluidCoupledMappingForPlasmaDynamics& rThis)
{
...
#endif // KRATOS_BINBASED_DEM_FLUID_COUPLED_MAPPING_FOR_PLASMA_DYNAMICS  defined
