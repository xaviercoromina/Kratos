...
def RenumberNodesIdsToAvoidRepeating(fluid_model_part, dem_model_part, rigid_faces_model_part):
    ...
    if must_renumber:
        ...
        Logger.PrintWarning("PlasmaDynamics","WARNING!, the DEM model part and the fluid model part have some ID values in common. Renumbering...")
        ...
        Logger.PrintWarning("PlasmaDynamics","The DEM model part and the fem-DEM model parts Ids have been renumbered")
...
class FluidFractionFieldUtility:
    ...
    def AddFluidFractionField(self):
        ...
        Logger.PrintInfo()
        Logger.PrintInfo("PlasmaDynamics",'Adding Imposed Fluid Fraction Fields...')
        Logger.PrintInfo()
        Logger.Flush()
        ...
        for field in self.field_list:
            ...
            Logger.PrintInfo("PlasmaDynamics",'field number', count, ':')
            Logger.PrintInfo()
            Logger.PrintInfo("PlasmaDynamics",vars(field))
            Logger.PrintInfo()
...
def ApplySimilarityTransformations(fluid_model_part, transformation_type, mod_over_real):
    ...
    if transformation_type == 0:
        ...
        Logger.PrintWarning("PlasmaDynamics",'***\n\nWARNING!, applying similarity transformations to the problem fluid variables')
        Logger.PrintWarning("PlasmaDynamics",'The particles diameters quotient is\n')
        Logger.PrintWarning("PlasmaDynamics",'D_model / D_real =', mod_over_real)
        Logger.PrintWarning("PlasmaDynamics",)
...
def FunctionsCalculator(domain_size=3):
    if domain_size == 2:
        custom_functions_tool = PlasmaDynApp.CustomFunctionsCalculator2D()

    elif domain_size == 3:
        custom_functions_tool = PlasmaDynApp.CustomFunctionsCalculator3D()
...
class PostUtils:
    ...
    def Writeresults(self, time):
        ...
        Logger.PrintInfo("PlasmaDynamics","*******************  PRINTING RESULTS FOR GID  ***************************")
        Logger.Flush()
