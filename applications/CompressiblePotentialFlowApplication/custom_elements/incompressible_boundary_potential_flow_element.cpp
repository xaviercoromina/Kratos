...
#include "incompressible_boundary_potential_flow_element.h"
...
Element::Pointer IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::Create(
    IndexType NewId, NodesArrayType const& ThisNodes, PropertiesType::Pointer pProperties) const
{
    KRATOS_TRY
    return Kratos::make_intrusive<IncompressibleBoundaryPotentialFlowElement>(
...
template <int Dim, int NumNodes>
Element::Pointer IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::Clone(
...
// template <int Dim, int NumNodes>
// void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::CalculateLocalSystem(
//     MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo)
// {
//     const IncompressibleBoundaryPotentialFlowElement& r_this = *this;
    ...
    MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo)
{
    ...
    if(wake == 0)//normal element (non-wake) - eventually an embedded
    {
        ...
        noalias(rLeftHandSideMatrix) = free_stream_density*data.vol * prod(data.DN_DX, trans(data.DN_DX));
    ...
    else //it is a wake element
    {
        ...
        Matrix lhs_total_wake = ZeroMatrix(NumNodes,NumNodes);
        ...
        // lhs_total_wake = length*prod(positive_side_interface_sh_func, trans(positive_side_interface_sh_func));
        ...
        double penalty=1000.0;//rCurrentProcessInfo[INITIAL_PENALTY];
        double alpha=1000.0;//rCurrentProcessInfo[PENALTY_COEFFICIENT];
        double geometry_angle=5;//rCurrentProcessInfo[MIU];
        ...
        noalias(lhs_plus_plus_alpha)  = alpha*data.vol*free_stream_density*prod(data.DN_DX, trans(data.DN_DX));
        noalias(lhs_plus_minus_alpha) = -alpha*data.vol*free_stream_density*prod(data.DN_DX, trans(data.DN_DX));
        noalias(lhs_minus_minus_alpha)= alpha*data.vol*free_stream_density*prod(data.DN_DX, trans(data.DN_DX));
        noalias(lhs_minus_plus_alpha) = -alpha*data.vol*free_stream_density*prod(data.DN_DX, trans(data.DN_DX));
        noalias(lhs_total) = free_stream_density*data.vol*prod(data.DN_DX, trans(data.DN_DX));
        for(unsigned int i=0; i<nsubdivisions; ++i)
        {
            if(PartitionsSign[i] > 0){
                noalias(lhs_positive) += Volumes[i]* free_stream_density*prod(data.DN_DX, trans(data.DN_DX));
                // noalias(lhs_plus_plus_alpha) += alpha*lhs_positive;
                // noalias(lhs_plus_minus_alpha) += -alpha*lhs_positive;
                noalias(lhs_kutta_positive) += Volumes[i] * free_stream_density*prod(test,trans(test));
            }
            else{
                noalias(lhs_negative) += Volumes[i]* free_stream_density*prod(data.DN_DX, trans(data.DN_DX));
                // noalias(lhs_minus_minus_alpha) += alpha*lhs_negative;
                // noalias(lhs_minus_plus_alpha) += -alpha*lhs_negative;
                noalias(lhs_kutta_negative) += Volumes[i] * free_stream_density*prod(test,trans(test));
            }
        }
        if(r_this.Is(STRUCTURE))//false
        {
            ...
            for(unsigned int i=0; i<NumNodes; ++i)
            {
                ...
                // if (true){
                    ...
                    for(unsigned int j=0; j<NumNodes; ++j)
                    {
                        ...
                        //     rLeftHandSideMatrix(i,j+NumNodes)          =  -lhs_total_wake(i,j);
                        // else if (data.distances[i] > 0.0)
                        //     rLeftHandSideMatrix(i+NumNodes,j)          =  -lhs_total_wake(i,j);
                    }
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::CalculateRightHandSide(
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::CalculateLeftHandSide(
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::EquationIdVector(
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::GetDofList(DofsVectorType& rElementalDofList,
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::FinalizeSolutionStep(const ProcessInfo& rCurrentProcessInfo)
{
    bool active = true;
...
template <int Dim, int NumNodes>
int IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::Check(const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::CalculateOnIntegrationPoints(
...
template <int Dim, int NumNodes>
std::string IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::Info() const
{
    std::stringstream buffer;
    buffer << "IncompressibleBoundaryPotentialFlowElement #" << Id();
    return buffer.str();
}

template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::PrintInfo(std::ostream& rOStream) const
{
    rOStream << "IncompressibleBoundaryPotentialFlowElement #" << Id();
}

template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::PrintData(std::ostream& rOStream) const
{
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::GetWakeDistances(array_1d<double, NumNodes>& distances) const
{
    noalias(distances) = GetValue(WAKE_ELEMENTAL_DISTANCES);
}

template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::GetEquationIdVectorNormalElement(EquationIdVectorType& rResult) const
{
    for (unsigned int i = 0; i < NumNodes; i++)
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::GetEquationIdVectorKuttaElement(EquationIdVectorType& rResult) const
{
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::GetEquationIdVectorWakeElement(EquationIdVectorType& rResult) const
{
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::GetDofListNormalElement(DofsVectorType& rElementalDofList) const
{
    for (unsigned int i = 0; i < NumNodes; i++)
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::GetDofListKuttaElement(DofsVectorType& rElementalDofList) const
{
    // Kutta elements have only negative part
    for (unsigned int i = 0; i < NumNodes; i++)
    {
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::GetDofListWakeElement(DofsVectorType& rElementalDofList) const
{
    array_1d<double, NumNodes> distances;
    GetWakeDistances(distances);
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::CalculateLocalSystemNormalElement(
    MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo)
{
    if (rLeftHandSideMatrix.size1() != NumNodes || rLeftHandSideMatrix.size2() != NumNodes)
        rLeftHandSideMatrix.resize(NumNodes, NumNodes, false);
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::CalculateLocalSystemWakeElement(
    MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo)
{
    // Note that the lhs and rhs have double the size
    ...
    if (this->Is(STRUCTURE))
    {
        ...
        AssignLocalSystemWakeElement(rLeftHandSideMatrix, lhs_total, lhs_positive, lhs_negative, data);
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::CalculateLocalSystemSubdividedElement(
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::ComputeLHSGaussPointContribution(
    const double weight,
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::AssignLocalSystemSubdividedElement(
    ...
    const ElementalData<NumNodes, Dim>& data) const
{
    for (unsigned int i = 0; i < NumNodes; ++i)
    {
        ...
        if (GetGeometry()[i].GetValue(TRAILING_EDGE))
        {
            ...
            AssignLocalSystemWakeNode(rLeftHandSideMatrix, lhs_total, lhs_positive, lhs_negative, data, i);
    }
}

template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::AssignLocalSystemWakeElement(
    MatrixType& rLeftHandSideMatrix,
    BoundedMatrix<double, NumNodes, NumNodes>& lhs_total,
    BoundedMatrix<double, NumNodes, NumNodes>& lhs_positive,
    BoundedMatrix<double, NumNodes, NumNodes>& lhs_negative,
    const ElementalData<NumNodes, Dim>& data) const
{
    for (unsigned int row = 0; row < NumNodes; ++row)
        AssignLocalSystemWakeNode(rLeftHandSideMatrix, lhs_total, lhs_positive, lhs_negative, data, row);
}

template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::AssignLocalSystemWakeNode(
    MatrixType& rLeftHandSideMatrix,
    BoundedMatrix<double, NumNodes, NumNodes>& lhs_total,
    BoundedMatrix<double, NumNodes, NumNodes>& lhs_positive,
    BoundedMatrix<double, NumNodes, NumNodes>& lhs_negative,
    ...
    unsigned int& row) const
{
    ...
    {
        rLeftHandSideMatrix(row, column) = lhs_positive(row, column) + penalty*lhs_total(row, column);
        rLeftHandSideMatrix(row + NumNodes, column + NumNodes) = lhs_negative(row, column) + penalty*lhs_total(row, column);
    }

    // Applying wake condition on the AUXILIARY_VELOCITY_POTENTIAL dofs
    if (data.distances[row] < 0.0)
        for (unsigned int column = 0; column < NumNodes; ++column)
            rLeftHandSideMatrix(row, column + NumNodes) = -penalty*lhs_total(row, column); // Side 1
    else if (data.distances[row] > 0.0)
        for (unsigned int column = 0; column < NumNodes; ++column)
            rLeftHandSideMatrix(row + NumNodes, column) = -penalty*lhs_total(row, column); // Side 2
}

template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::ComputePotentialJump(const ProcessInfo& rCurrentProcessInfo)
{
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::ComputeElementInternalEnergy()
{
...
template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::save(Serializer& rSerializer) const
{
    KRATOS_SERIALIZE_SAVE_BASE_CLASS(rSerializer, Element);
}

template <int Dim, int NumNodes>
void IncompressibleBoundaryPotentialFlowElement<Dim, NumNodes>::load(Serializer& rSerializer)
{
...
template class IncompressibleBoundaryPotentialFlowElement<2, 3>;
template class IncompressibleBoundaryPotentialFlowElement<3, 4>;
