...
class MassResponseFunction(ResponseFunction):
    ...
    def CalculateValue(self) -> float:
        return KratosOA.ResponseUtilities.MassResponseUtilities.CalculateMass(self.model_part)

    def CalculateSensitivity(self, sensitivity_variable: Union[Kratos.DoubleVariable, Kratos.Array1DVariable3], sensitivity_model_part: Kratos.ModelPart):
        if sensitivity_variable == Kratos.SHAPE_SENSITIVITY:
            KratosOA.ResponseUtilities.MassResponseUtilities.CalculateMassShapeSensitivity(sensitivity_model_part, sensitivity_variable)
        elif sensitivity_variable == KratosOA.DENSITY_SENSITIVITY:
            KratosOA.ResponseUtilities.MassResponseUtilities.CalculateMassDensitySensitivity(sensitivity_model_part, sensitivity_variable)
        elif sensitivity_variable == KratosOA.THICKNESS_SENSITIVITY:
            KratosOA.ResponseUtilities.MassResponseUtilities.CalculateMassThicknessSensitivity(sensitivity_model_part, sensitivity_variable)
        elif sensitivity_variable == KratosOA.CROSS_AREA_SENSITIVITY:
            KratosOA.ResponseUtilities.MassResponseUtilities.CalculateMassCrossAreaSensitivity(sensitivity_model_part, sensitivity_variable)
        else:
