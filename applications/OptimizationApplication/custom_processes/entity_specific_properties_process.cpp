    ...
    : mrModel(rModel)
{
    ...
    const auto& r_container_type = rParameters["container_type"].GetString();

    if (r_container_type == "conditions") {
        mIsConditions = true;
    } else if (r_container_type == "elements") {
        mIsElements = true;
    } else if (r_container_type == "all") {
        mIsConditions = true;
        mIsElements = true;
    } else {
        KRATOS_ERROR << "Unsupported container type = \"" << r_container_type
                     << "\" requested for " << mModelPartName
                     << ". Followings are supported container types:"
                     << "\n\tconditions"
                     << "\n\telements"
                     << "\n\tall";
    }
