...
class RomBasisProcess(KratosMultiphysics.Process):
    def __init__(self, Model, parameters):
        ...
        if self.parameters["build_petrov_galerkin"].GetBool():
            self.petrov_galerkin_flag = True
            self.displacement_flag = False
        ...
        self.time_step_solution_container_assembled_residuals = []
    ...
    def ExecuteInitialize(self):
        ...
        """
        if  self.stress_flag:
            self.BuildSimpleExtrapolationOperator() # Build a sparse extrapolation (smoothing) operator for stresses (from GP to nodes)
        ...
        if self.petrov_galerkin_flag:
            residual_parameters = KratosMultiphysics.Parameters("""
            {
                "nodal_unknowns" : ["REACTION_X","REACTION_Y","REACTION_Z"],
                "number_of_dofs" : 1
            }""")
            self.ResidualMainModelPartUtility = romapp.RomModelPartUtility(self.model_part,residual_parameters,KratosMultiphysics.ResidualBasedIncrementalUpdateStaticScheme())
    ...
    def ExecuteFinalizeSolutionStep(self):
        ...
        if self.petrov_galerkin_flag:
            ArrayOfAssembledResiduals = self.ResidualMainModelPartUtility.GetAssembledResiduals()
            self.time_step_solution_container_assembled_residuals.append(np.array(ArrayOfAssembledResiduals, copy=False))
    ...
    def ExecuteFinalize(self):
        ...
        #Create a new basis with the assembled residuals for a Petrov-Galerkin projection
        if self.petrov_galerkin_flag:
            with open('RomParameters.json') as s:
                basis_POD = json.load(s) 
            N_Dof_per_node = len(basis_POD["rom_settings"]["nodal_unknowns"])
            N_nodes = len(basis_POD["nodal_modes"])
            N_Dofs = int(N_Dof_per_node*N_nodes)
            N_Dofs_rom = basis_POD["rom_settings"]["number_of_rom_dofs"]
            u = np.zeros((N_Dofs,N_Dofs_rom))
            counter_in = 0
            for key in basis_POD["nodal_modes"].keys(): 
                counter_fin = counter_in + N_Dof_per_node
                u[counter_in:counter_fin,:] = np.array(basis_POD["nodal_modes"][key])
                counter_in = counter_fin
            SnapshotMatrix_assembled_residuals = self.EvaluateQuantityOfInterest(self.time_step_solution_container_assembled_residuals)
            #######ERASE: ONLY TO COMPARE RESIDUALS WITH REACTIONS#######
            with open('test.npy', 'wb') as f:
                np.save(f, SnapshotMatrix_assembled_residuals)
            ############################################################
            residual_tolerance = self.parameters["residual_basis_tolerance"].GetDouble()
            # u_petrov = u
            u_petrov = self.BuildOrthogonalComplementBasis(SnapshotMatrix_assembled_residuals,u,residual_tolerance)###Aquí nos quedamos
            basis_POD["Petrov_Galerkin_basis"]={"rom_settings":{},"nodal_modes":{}}
            basis_POD["Petrov_Galerkin_basis"]["rom_settings"]["nodal_unknowns"] = ["RESIDUAL_X","RESIDUAL_Y","RESIDUAL_Z"]
            basis_POD["Petrov_Galerkin_basis"]["rom_settings"]["number_of_rom_dofs"] = np.shape(u_petrov)[1]
            Dimensions = len(basis_POD["Petrov_Galerkin_basis"]["rom_settings"]["nodal_unknowns"])
            N_nodes=np.shape(u_petrov)[0]/Dimensions
            N_nodes = int(N_nodes)
            node_Id=np.linspace(1,N_nodes,N_nodes)
            i = 0
            for j in range (0,N_nodes):
                basis_POD["Petrov_Galerkin_basis"]["nodal_modes"][int(node_Id[j])] = (u_petrov[i:i+Dimensions].tolist())
                i=i+Dimensions
    ...
    def BuildSimpleExtrapolationOperator(self):
        #TODO in C++
        #Builds an smoothing operator given that all elements have only 1 GAUSS POINT, therefore, the stress is the same in all the nodes in the element.
    ...
    def BuildOrthogonalComplementBasis(self,SnapshotMatrix,Orthogonal_Basis,basis_tolerance):
        for j in range(10):
            aux = Orthogonal_Basis.T@SnapshotMatrix
            aux = Orthogonal_Basis@aux
            SnapshotMatrix = SnapshotMatrix-aux
        # u_petrov,_,_,_= RandomizedSingularValueDecomposition().Calculate(SnapshotMatrix[:,:],basis_tolerance)
        u_petrov,_,_ = self.rSVD(SnapshotMatrix,5,0,0)
        u = np.concatenate((Orthogonal_Basis,u_petrov),axis=1)
        Q, _ = np.linalg.qr(u,mode='reduced')
        return Q
